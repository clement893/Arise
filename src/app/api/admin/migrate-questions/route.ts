...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ 
      ...
      counts: {
        tki: tkiQuestions.length,
        wellness: wellnessQuestions.length,
        '360-self': selfAssessmentQuestions.length,
      }
    });
  } catch (error) {
    console.error('Error during migration:', error);
    return NextResponse.json(
      { error: 'Failed to migrate questions', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
